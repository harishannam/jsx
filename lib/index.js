...
exports.default = function (_ref) {
  ...
  var initConfig = function initConfig(path, state) {
    var _state$opts = state.opts,
        ...
        _state$opts$useGuid = _state$opts.useGuid,
        useGuid = _state$opts$useGuid === undefined ? true : _state$opts$useGuid;
  ...
  var visitJSXElement = function visitJSXElement(path, state) {
    ...
    var _JSXMemberExpression = (0, _utils.transformOnType)({
    ...
    var JSXElementName = (0, _utils.transformOnType)({
    ...
    var JSXAttributeName = (0, _utils.transformOnType)({
      JSXIdentifier: JSXIdentifier,
      JSXNamespacedName: JSXNamespacedName,
      JSXMemberExpression: _JSXMemberExpression
    });

    var JSXAttributeValue = (0, _utils.transformOnType)({
    ...
    var JSXElement = function JSXElement(node) {
      var objExpression = [t.objectProperty(t.identifier(nameProperty), JSXElementName(node.openingElement.name)), t.objectProperty(t.identifier(attributesProperty), JSXAttributes(node.openingElement.attributes)), t.objectProperty(t.identifier(childrenProperty), JSXChildren(node.children))];

      if (state.opts.useGuid) {
        objExpression.push(t.objectProperty(t.identifier(uniqueId), t.stringLiteral(_uuidmanager.UUIDManager.generate())));
      }

      return jsxObjectTransformer(t.objectExpression(objExpression));
    };

    var JSXChild = (0, _utils.transformOnType)({
      JSXText: JSXText,
    ...
    var JSXChildren = function JSXChildren(nodes) {
      ...
      .reduce(function (children, child) {
        ...
        if (child.type === 'JSXEmptyExpression') {
          return (0, _utils.toConsumableArray)(children);
        }
...
var _uuidmanager = require('./uuidmanager.js');
